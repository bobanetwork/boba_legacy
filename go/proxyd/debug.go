...
func debugResult(ctx context.Context, method string, defaultRes *RPCRes, debugRes *RPCRes) {
	...
	if !ok {
		log.Error("Failed to decode debugResult", "method", method, "debugResult", debugResult, "req_id", reqID)
	}
	defaultResult, ok := defaultRes.Result.(string)
	if !ok {
		log.Error("Failed to decode defaultResult", "method", method, "defaultResult", defaultResult, "req_id", reqID)
	}
...
func debugLogs(ctx context.Context, method string, defaultRes *RPCRes, debugRes *RPCRes) {
	...
	if !ok {
		log.Error("Failed to decode debugLogs", "method", method, "debugLogs", debugLogs, "req_id", reqID)
	}
	defaultLogs, ok = defaultRes.Result.(*types.Log)
	if !ok {
		log.Error("Failed to decode defaultLogs", "method", method, "defaultLogs", defaultLogs, "req_id", reqID)
	}
