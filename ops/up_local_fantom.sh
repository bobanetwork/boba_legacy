...
if [[ $BUILD == 1 ]]; then
  ...
  docker-compose -f docker-compose-fantom.yml build --parallel -- boba_message-relayer-fast gas_oracle boba_deployer
  docker-compose -f docker-compose-fantom.yml build --parallel -- fraud-detector monitor
  docker-compose -f docker-compose-fantom.yml build --parallel -- verifier replica
elif [[ $BUILD == 0 ]]; then
