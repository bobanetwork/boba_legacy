...
const counterPartProvider = new providers.JsonRpcProvider(
  cfg['bridgeCounterpartUrl']
) // for testing the bridging

const BOBAL2Address = '0xF5B97a4860c1D81A1e915C40EcCB5E4a5E6b8309'
const BobaTuringCreditRinkebyAddress =
...
console.log = (message?: any, ...optionalParams: any[]) =>
  oldConsole(`--> ${message}`, optionalParams)
...
describe('Turing bridgeable NFT Random 256', function () {
  before(async () => {
    ...
    console.log(
      'adding your ERC721 as PermittedCaller to TuringHelper',
      res1.events[0].data
    )
    ...
    turingCredit = getContractFactory('BobaTuringCredit', testWallet).attach(
      BobaTuringCreditRinkebyAddress
    )
    ...
    L2NFTBridgeContract = await Factory__L2NFTBridge.deploy(gasOverride)

    const txInit = await L2NFTBridgeContract.initialize(
      L2BridgeMockContract.address,
      '0x1234123412341234123412341234123412341234' // NOTE: Using a dummy L1 bridge address for testing
    )
    await txInit.wait()
  ...
  it('Should register and fund your Turing helper contract in turingCredit', async () => {
    ...
    const postBalance = await turingCredit.prepaidBalance(helper.address)
  })
  ...
  it('Your ERC721 contract should be whitelisted', async () => {
    ...
    console.log(
      'ERC721 contract whitelisted in TuringHelper (1 = yes)?',
      result
    )
  })
  ...
  it('should mint 3 NFTs with random attributes', async () => {
    ...
    console.log(
      `Trying to mint ${amountNFTsToMint} NFTs for ${ethers.utils.formatEther(
        mintingPrice
      )} ETH each.`
    )
    const tr = await erc721.mint(amountNFTsToMint, {
      gasLimit: 1000000,
      value: mintingPrice * amountNFTsToMint,
    })
    ...
    const tokenIDs = [
      mintedEvents[0]?.args[0],
      mintedEvents[1]?.args[0],
      mintedEvents[2]?.args[0],
    ]
    // very very low probability that all three combinations are linearly assigned when using random tokenIDs
    expect(
      Math.abs(tokenIDs[0] - tokenIDs[1]) > 1 ||
        Math.abs(tokenIDs[1] - tokenIDs[2]) > 1 ||
        Math.abs(tokenIDs[2] - tokenIDs[0]) > 1,
      "TokenIDs don't seem to be assigned randomly."
    )

    console.log('Turing NFT =', res)

    console.log(
      `Trying to mint 1 NFTs for ${ethers.utils.formatEther(
        mintingPrice
      )} ETH each, should fail as max mint per wallet reached.`
    )
    const trFail = await erc721.mint(1, {
      gasLimit: 1000000,
      value: mintingPrice,
    })
    const resFail = trFail.wait()
    ...
    const trDif = await erc721DifferentSigner.mint(difWalletAmount, {
      gasLimit: 1000000,
      value: mintingPrice * difWalletAmount,
    })
    const resDif = await trDif.wait()
    expect(resDif).to.be.ok

    console.log(
      `Trying to mint 1 NFTs for ${ethers.utils.formatEther(
        mintingPrice
      )} ETH each, should fail as max supply reached.`
    )
    const trFail2 = await erc721DifferentSigner.mint(1, {
      gasLimit: 1000000,
      value: mintingPrice,
    })
    const resFail2 = trFail2.wait()
  ...
  it('should get onchain metadata', async () => {
    ...
    const uri = await erc721.tokenURI(tokenId, gasOverride)
  ...
  it('should return bridgeExtraData', async () => {
    ...
    const decoded_1 = utils.defaultAbiCoder.decode(
      ['uint256'],
      bridgeExtraData_1
    )
    const decoded_2 = utils.defaultAbiCoder.decode(
      ['uint256'],
      bridgeExtraData_2
    )
  ...
  it('should have different metadata', async () => {
    ...
    const uri1 = await erc721.tokenURI(tokenId_1, gasOverride)
    const uri2 = await erc721.tokenURI(tokenId_2, gasOverride)
    const decodedMetadata1 = JSON.parse(
      Buffer.from(uri1.substring(uri1.indexOf(',') + 1), 'base64').toString()
    )
    const decodedMetadata2 = JSON.parse(
      Buffer.from(uri2.substring(uri2.indexOf(',') + 1), 'base64').toString()
    )
    ...
    expect(decodedMetadata1['attributes'][0].value).to.be.not.equal(
      decodedMetadata2['attributes'][0].value,
      'Metadata EYE equals previous mint'
    )
    expect(decodedMetadata1['attributes'][1].value).to.be.not.equal(
      decodedMetadata2['attributes'][1].value,
      'Metadata BODY equals previous mint'
    )
    expect(decodedMetadata1['attributes'][2].value).to.be.not.equal(
      decodedMetadata2['attributes'][2].value,
      'Metadata EXTRA equals previous mint'
    )
  })
  ...
  it('bridge NFT to L1', async () => {
    ...
    console.log(
      `Approved NFT to bridge, nftL2 ${erc721.address}, bridgeL2 ${L2NFTBridgeContract.address}`
    )
