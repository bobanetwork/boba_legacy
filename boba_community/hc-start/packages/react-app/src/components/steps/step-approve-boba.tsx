...
import { L2GovernanceERC20 } from "@hc/contracts/gen/types";
...
export const StepApproveBoba = (props: IStepApproveBobaProps) => {
  ...
  if (newTransaction) {
    ...
    <SmallerParagraph>You need to prepay your HybridComputeHelper with some BOBA tokens. How many off-chain calls do you want
    ...
    <CustomButton style={{ marginTop: 14 }} disabled={!account || loadingState || !hasEnoughBOBA}
                  onClick={async () => {
              newTransaction = true;
              await approveBoba(addresses.HybridComputeHelperFactory, amountBobaTokensToUseWei);
