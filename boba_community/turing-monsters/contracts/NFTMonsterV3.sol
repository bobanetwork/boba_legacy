...
import "./AddSaleStart.sol";


contract NFTMonsterV3 is IERC2981, ERC721Burnable, ERC721Pausable, RandomlyAssigned, AddRecover, AddOnChainMetaData, AddSaleStart {
    ...
    ERC721Burnable public firstGenNFT;
    ...
    constructor(string memory name_, string memory symbol_, uint256 maxNFTs_,
        ...
        address firstGenNFTAddress_, uint256 publicSaleStart_)
    ...
    AddSaleStart(publicSaleStart_)
    {
        _pause();
        projectOwners = creatorAddresses_;
        firstGenNFT = ERC721Burnable(firstGenNFTAddress_);
    }
    ...
    function mint(uint256 _count) external payable saleIsOpen {
        require(_count > 0 && _count <= MAX_MINT, "Mint not within boundaries");

        bool isPublicSale = saleStarted();
        if (!isPublicSale) {
            require(firstGenNFT.balanceOf(_msgSender()) >= _count, "Private: Not enough first gen NFTs");
        }
        ...
        amountMinted[_msgSender()] = amountMinted[_msgSender()] + _count;
        require(amountMinted[_msgSender()] <= MAX_MINT, "Wallet limit reached");
    ...
    /// @dev Only callable once to protect investors.
