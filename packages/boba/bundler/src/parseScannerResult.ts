...
export function parseScannerResult(
    ...
    ([entityTitle, entStakes], index) => {
      ...
      Object.entries(access).forEach(([addr, { reads, writes }]) => {
        ...
        function associatedWith(
          slot: string,
          address: string,
          eSlots: { [addr: string]: Set<string> }
        ): boolean {
          const addrPadded = hexZeroPad(address, 32).toLowerCase()
          if (slot === addrPadded) {
            return true
          }
          const k = eSlots[addr]
        ...
        function nameAddr(address: string, currentEntity: string): string {
          const [title] =
            Object.entries(stakeInfoEntities).find(
              // eslint-disable-next-line @typescript-eslint/no-shadow
              ([title, info]) =>
                info?.addr.toLowerCase() === address.toLowerCase()
            ) ?? []
  ...
  const addresses = tracerResults.numberLevels.reduce(
    (acc, val) => acc.concat(Object.keys(val.contractSize)),
    []
  )
