...
import { AddressZero, rethrowError } from '@boba/bundler_utils'
...
import { calcPreVerificationGas } from '../../bundler_sdk/dist/src/calcPreVerificationGas'
...
export class UserOpMethodHandler {
  constructor (
    ...
    readonly entryPoint: EntryPoint // readonly bundlerHelper: BundlerHelper
  ) {}
  ...
  async isGeth (): Promise<boolean> {
    if (this.clientVersion == null) {
      this.clientVersion = await (this.provider as JsonRpcProvider).send(
        'web3_clientVersion',
        []
      )
    }
  ...
  async validateUserOperation (
    ...
    const fieldNames =
      'sender,nonce,initCode,callData,callGasLimit,verificationGasLimit,preVerificationGas,maxFeePerGas,maxPriorityFeePerGas,paymasterAndData'
  ...
  async simulateUserOp (
    ...
    if (
      entryPointInput.toLowerCase() !== this.config.entryPoint.toLowerCase()
    ) {
      throw new Error(
    ...
    const revert = await this.entryPoint.callStatic
    ...
    if (await this.isGeth()) {
      ...
      const simulationGas = BigNumber.from(50000).add(
        userOp.verificationGasLimit
      )

      const result: BundlerCollectorReturn = await debug_traceCall(
        provider,
        {
      ...
      const paymaster =
        (userOp.paymasterAndData?.length ?? 0) >= 42
          ? userOp.paymasterAndData.toString().slice(0, 42)
          : undefined
      ...
      Object.keys(validateOpcodes).forEach((opcode) =>
        requireCond(
          ...
          `wallet uses banned opcode: ${opcode}`,
          32501
        )
      )
      Object.keys(validatePaymasterOpcodes).forEach((opcode) =>
        requireCond(
          !bannedOpCodes.has(opcode),
          `paymaster uses banned opcode: ${opcode}`,
          32501,
          { paymaster }
        )
      )
      if (userOp.initCode.length > 2) {
        requireCond(
          (validateOpcodes.CREATE2 ?? 0) <= 1,
          'initCode with too many CREATE2',
          32501
        )
      } else {
        requireCond(
          (validateOpcodes.CREATE2 ?? 0) < 1,
          'banned opcode: CREATE2',
          32501
        )
      }
      requireCond(
        (validatePaymasterOpcodes.CREATE2 ?? 0) < 1,
        'paymaster uses banned opcode: CREATE2',
        32501,
        { paymaster }
      )
    }
  ...
  async sendUserOperation (
    ...
    console.log(
      `UserOperation: Sender=${
        userOp.sender
      } EntryPoint=${entryPointInput} Paymaster=${hexValue(
    ...
    await this.entryPoint
      .handleOps([userOp], beneficiary, { gasLimit })
      .catch(rethrowError)
