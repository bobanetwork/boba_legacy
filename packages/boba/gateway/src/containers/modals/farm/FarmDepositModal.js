...
class FarmDepositModal extends React.Component {
  ...
  async componentDidUpdate(prevState) {
    ...
    if (prevState.setup.netLayer !== netLayer) {
      ...
      this.setState({ netLayerNativeToken: netLayer === 'L1' ? networkService.L1NativeTokenSymbol: 'BOBA' })
    }
    ...
    if (!isEqual(prevState.farm.stakeToken, stakeToken)) {
      ...
      if ( stakeToken.symbol !== this.state.netLayerNativeToken ) {
        ...
        // Set to some very big number to approved allowance in case of netLayerNativeToken.
        // There is no need to query allowance for depositing netLayerNativeToken on the L1 or the L2
        this.props.dispatch({
  ...
  async getMaxTransferValue() {
    ...
    if (netLayer === 'L2') {
      ...
      // both netLayerNativeToken and BOBA have 18 decimals so this is safe
      if (stakeToken.symbol === netLayerNativeToken) {
        // we are staking netLayerNativeToken
        // since MetaMask does not know about BOBA, we need to subtract the netLayerNativeToken fee
        // regardless of how we are paying, otherwise will get an error in MetaMask
        max_BN = max_BN.sub(cost_BN)
        // minimum netLayerNativeToken in account
        max_BN = max_BN.sub(BigNumber.from(toWei_String(0.002, 18)))
      }
      else if (stakeToken.symbol === networkService.L1NativeTokenSymbol && bobaFeeChoice) {
  ...
  render() {
    ...
    if (Number(approvedAllowance) > 0 &&
      ...
      //do not need to approve netLayerNativeToken
      allowanceGTstake = true
    }
