...
/** @dev No separate exit/deposit step needed, since Teleportation works in all directions. */
function TeleportationStepFast({ handleClose, token, isBridge, openTokenPicker }) {
    ...
    <>
      <Box>
        ...
        {(Number(LPRatio) < 0.10 && Number(value) > Number(balanceSubPending) * 0.90) && (
          <Typography variant="body2" sx={{mt: 2, color: 'red'}}>
            The disburser's balance and balance/liquidity ratio are too low.
        ...
        {(Number(LPRatio) < 0.10 && Number(value) <= Number(balanceSubPending) * 0.90) && (
          <Typography variant="body2" sx={{mt: 2, color: 'red'}}>
            The disburser's balance/liquidity ratio (of {Number(LPRatio).toFixed(2)}) is too low.
            Please use the classic bridge.
          </Typography>
        )}
        ...
        {(Number(LPRatio) >= 0.10 && Number(value) > Number(balanceSubPending) * 0.90) && (
          <Typography variant="body2" sx={{mt: 2, color: 'red'}}>
            The disburser's balance (of {Number(balanceSubPending).toFixed(2)}) is too low.
      ...
      <WrapperActionsModal>
        ...
        <Button
          onClick={teleportAsset}
          color='primary'
          variant='contained'
          loading={loading}
          tooltip={loading ? "Your transaction is still pending. Please wait for confirmation." : "Click here to bridge your funds to the desired network"}
          disabled={!validValue}
...
export default React.memo(TeleportationStepFast)
