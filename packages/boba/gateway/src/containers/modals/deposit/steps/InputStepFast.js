  ...
  if(cost && Number(cost) > 0) {
    ...
    if (token.symbol !== networkService.L1NativeTokenSymbol) {
      if(Number(cost) > Number(feeBalance)) {
        warning = true
        ETHstring = `WARNING: your L1 ${networkService.L1NativeTokenSymbol} balance of ${Number(feeBalance).toFixed(4)} is not sufficient to cover the estimated gas.
        <br/>THIS TRANSACTION WILL FAIL.`
      }
      else if(Number(cost) > Number(feeBalance) * 0.96) {
        warning = true
        ETHstring = `CAUTION: your L1 ${networkService.L1NativeTokenSymbol} balance of ${Number(feeBalance).toFixed(4)} is very close to the estimated cost.
        <br/>THIS TRANSACTION MIGHT FAIL. It would be safer to have slightly more ${networkService.L1NativeTokenSymbol} in your L1 wallet to cover gas.`
      }
    }
    ...
    if (token.symbol === networkService.L1NativeTokenSymbol) {
      if((Number(value) + Number(cost)) > Number(feeBalance)) {
        ...
        warning = true
        ETHstring = `CAUTION: your L1 ${networkService.L1NativeTokenSymbol} balance of ${Number(feeBalance).toFixed(4)} is very close to the estimated total.
    ...
    <>
      <Box>
        ...
        <BridgeFee
          ...
          estFee={`${Number(cost).toFixed(4)} ${networkService.L1NativeTokenSymbol}`}
        />
