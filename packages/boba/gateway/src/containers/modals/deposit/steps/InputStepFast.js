...
function InputStepFast({ handleClose, token, isBridge, openTokenPicker }) {
  ...
  function setAmount(value) {
    ...
    if (tooSmall || tooBig) {
      ...
      token.symbol === 'ETH' &&
      Number(cost) + Number(value) > Number(feeBalance)
    ) {
  ...
  if (cost && Number(cost) > 0) {
    if (token.symbol !== 'ETH') {
      if (Number(cost) > Number(feeBalance)) {
        warning = true
        ETHstring = `WARNING: your L1 ETH balance of ${Number(
          feeBalance
        ).toFixed(4)} is not sufficient to cover the estimated gas.
        <br/>THIS TRANSACTION WILL FAIL.`
      } else if (Number(cost) > Number(feeBalance) * 0.96) {
        warning = true
        ETHstring = `CAUTION: your L1 ETH balance of ${Number(
          feeBalance
        ).toFixed(4)} is very close to the estimated cost.
    ...
    if (token.symbol === 'ETH') {
      if (Number(value) + Number(cost) > Number(feeBalance)) {
        ...
        ETHstring = `WARNING: your L1 ETH balance of ${Number(
          feeBalance
        ).toFixed(4)} is not sufficient to cover this transaction.
        <br/>THIS TRANSACTION WILL FAIL.`
      } else if (Number(value) + Number(cost) > Number(feeBalance) * 0.96) {
        warning = true
        ETHstring = `CAUTION: your L1 ETH balance of ${Number(
          feeBalance
        ).toFixed(4)} is very close to the estimated total.
  ...
  return (
    <>
      <Box>
        ...
        {Number(LPRatio) < 0.1 &&
          Number(value) > Number(balanceSubPending) * 0.9 && (
            <Typography variant="body2" sx={{ mt: 2, color: 'red' }}>
              The {token.symbol} pool's balance and balance/liquidity ratio are
        ...
        {Number(LPRatio) < 0.1 &&
          Number(value) <= Number(balanceSubPending) * 0.9 && (
            <Typography variant="body2" sx={{ mt: 2, color: 'red' }}>
              The {token.symbol} pool's balance/liquidity ratio (of{' '}
        ...
        {Number(LPRatio) >= 0.1 &&
          Number(value) > Number(balanceSubPending) * 0.9 && (
            <Typography variant="body2" sx={{ mt: 2, color: 'red' }}>
              The {token.symbol} pool's balance (of{' '}
              {Number(balanceSubPending).toFixed(2)} including inflight bridges)
              is too low. Please use the classic bridge or reduce the amount.
            </Typography>
          )}
        ...
        {!isBridge && (depositLoading || approvalLoading) && (
          <Typography variant="body2" sx={{ mt: 2, color: 'green' }}>
            This window will automatically close when your transaction has been
        ...
        <Button
          ...
          tooltip={
            depositLoading
              ...
              : 'Click here to bridge your funds to L2'
          }
