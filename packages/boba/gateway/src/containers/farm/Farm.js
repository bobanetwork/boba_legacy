...
class Farm extends React.Component {
  ...
  constructor(props) {
    ...
    let initialViewLayer = `${process.env.REACT_APP_L1_NATIVE_TOKEN_SYMBOL} Pool`
  ...
  handleChange = (event, t) => {
    if (t === `${networkService.L1NativeTokenSymbol} Pool`) {
      this.setState({
        lpChoice: 'L1LP',
        poolTab: t
      })
      ...
      <S.EarnPageContainer>
        ...
        <Box sx={{ my: 1, width: '100%' }}>
          <S.EarnActionContainer sx={{ mb: 2, display: 'flex' }}>
            <S.PageSwitcher>
              <Typography
                className={poolTab === `${networkService.L1NativeTokenSymbol} Pool` ? 'active' : ''}
            ...
            <Tooltip
              title={
                <Typography variant="body2" sx={{ mt: 1, fontSize: '0.9em' }}>
                  <span style={{ fontWeight: '700' }}>Staking example</span>. When you stake 10 BOBA into the L2 pool, then the pool's liquidity and balance both increase by 10 BOBA.
                  <br /><br />
                  <span style={{ fontWeight: '700' }}>Fast Bridge example</span>. When a user bridges 10 BOBA from L1 to L2 using the fast bridge,
                  they send 10 BOBA to the L1 pool, increasing its balance by 10 BOBA. Next, 9.99 BOBA flow out from the L2 pool to the user's L2 wallet, completing the bridge.
                  Note that bridge operations do not change the pool's liquidity, but only its balance.
                  The difference between what was deposited into the L1 pool (10 BOBA) and what was sent
                  to the user on the L2 (9.99 BOBA), equal to 0.01 BOBA, is sent to the reward pool, for harvesting by stakers.
                  <br /><br />
