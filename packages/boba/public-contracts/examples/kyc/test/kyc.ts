...
import TuringHelperJson from "../../../../turing/artifacts/contracts/TuringHelper.sol/TuringHelper.json"
...
describe("Verify KYC for function calls", function () {
    ...
    it("contract should be whitelisted", async () => {
        const tr2 = await turingHelper.checkPermittedCaller(kyc.address, gasOverride)
    ...
    it("should return the helper address", async () => {
        let helperAddress = await kyc.turingHelper()
