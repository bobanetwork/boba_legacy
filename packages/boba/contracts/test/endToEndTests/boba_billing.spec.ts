...
import { Contract, Signer, BigNumber, utils, BigNumberish, ContractFactory } from 'ethers'
...
let transactionFee = utils.parseEther('1')
...
describe('L2 Billing Contract', async () => {
  before(async () => {
    ...
    L2BillingContract = await billingContract.deploy(L2Boba.address, treasuryAddress, transactionFee);
  ...
  describe('Collect fee', async () => {
    it('should fail when address is incorrect', async () => {
      await expect(L2BillingContract.collectFeeFrom('0x0000000000000000000000000000000000000000')).to.be.revertedWith(
        'account cannot be zero'
      )
    })

    it('should revert when having insufficient balance', async () => {
      await L2Boba.connect(signer2).approve(L2BillingContract.address, transactionFee);
      await expect(L2BillingContract.connect(signer2).collectFeeFrom(signer2Address)).to.be.revertedWith(
