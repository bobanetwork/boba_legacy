...
contract L2_BobaPortal {
  ...
  // round-trip health check protocol, by ensuring that the most recent rolling hash known to the
  // L1 contract was in fact generated by this L2 contract rather than a man-in-the-middle attacker.
  ...
  constructor(uint64 deployL1) {
    ...
    //fastList[0x1fA02b2d6A771842690194Cf62D91bdd92BfE28d] = true; // Proxy__L2LiquidityPool
     ...
     public
  {
    ...
    if(msgType == 1) {
      ...
      // Health check
      bytes32 checkHash = abi.decode(payload,(bytes32));
      
      // FIXME - debugging; remove
      require(sentHashes[checkHash] != 0, "Failed healthCheckHash");
      
      uint64 checkSeq = uint64(sentHashes[checkHash] >> 192);
      
      bytes32 responseHash = keccak256(abi.encodePacked(checkHash, checkSeq));
      bytes memory respMsg = abi.encode(responseHash);
      sendOut(0x80000004, 0, respMsg);
