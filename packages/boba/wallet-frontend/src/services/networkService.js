...
import bobaWatcherAxiosInstance from 'api/bobaWatcherAxios'
import addressAxiosInstance from 'api/addressAxios'
import addressBOBAAxiosInstance from 'api/addressBOBAAxios'
...
class NetworkService {
  ...
  async initializeAccounts( masterSystemConfig ) {
    ...
    try {
      ...
      if (masterSystemConfig === 'local') {
        ...
        try {
          resBOBA = await addressBOBAAxiosInstance('local').get()
        } catch (error) {
          console.log(error)
        }
        ...
        if (resBOBA !== null && resBase !== null) {
          addresses = { ...resBase.data, ...resBOBA.data }
        } else {
  ...
  async getTransactions() {
    ...
    const responseL2 = await bobaWatcherAxiosInstance(
      this.masterSystemConfig
    ...
    const responseL1pending = await bobaWatcherAxiosInstance(
      this.masterSystemConfig
  ...
  async getExits() {
    ...
    const response = await bobaWatcherAxiosInstance(
      this.masterSystemConfig
    ).post('get.l2.transactions', {
      address: this.account,
      fromRange:  0,
