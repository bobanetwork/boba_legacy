...
import { ManualDepositPaymaster__factory } from '../dist/types/factories/contracts/samples/'
...
describe('ManualDepositPaymaster', () => {
  ...
  before(async function () {
    ...
    paymaster = await new ManualDepositPaymaster__factory(ethersSigner).deploy(
      entryPoint.address
    )
  ...
  describe('addToken', () => {
    it('should not allow adding invalid token', async () => {
      ...
      ).to.be.revertedWith('DepositPaymaster: Invalid token')
    })
    it('should not allow zero priceRatio', async () => {
      ...
      ).to.be.revertedWith('DepositPaymaster: price ratio cannot be zero')
    })
    it('should not allow incorrect priceRatio ranges', async () => {
      ...
      ).to.be.revertedWith('DepositPaymaster: Invalid price ratio')
      await expect(
        paymaster.addToken(
          tokenSample.address,
          await token.decimals(),
      ...
      ).to.be.revertedWith('DepositPaymaster: min ratio cannot be zero')
      await expect(
        paymaster.addToken(
          tokenSample.address,
          await token.decimals(),
  ...
  describe('deposit', () => {
    ...
    it('should fail to deposit unsupported token', async () => {
      ...
      await expect(
        paymaster.addDepositFor(tokenSample.address, account.address, 100)
  ...
  describe('token params', () => {
    ...
    it('should not allow non owner to update priceRatio', async () => {
      const updatePriceRatio = await paymaster.populateTransaction
        .updatePriceRatio(token.address, 60)
        .then((tx) => tx.data!)

      await expect(
        account.execute(paymaster.address, 0, updatePriceRatio)
      ).to.revertedWith('Ownable: caller is not the owner')
    })
    it('should not allow non owner to update token params', async () => {
      const updateParams = await paymaster.populateTransaction
        .updateTokenParams(token.address, priceRatioDecimals, 10, 1000)
        .then((tx) => tx.data!)

      await expect(
        account.execute(paymaster.address, 0, updateParams)
      ).to.revertedWith('Ownable: caller is not the owner')
    })
    it('should allow only the owner to update token params and priceRatio', async () => {
      await paymaster.updateTokenParams(
        token.address,
        priceRatioDecimals,
        10,
        1000
      ...
      const priceRatio = (await paymaster.priceRatioInfo(token.address))
        .priceRatio
  ...
  describe('getTokenValueOfEth', () => {
    ...
    before(async () => {
      manualDepositPaymaster =
        await new TestTokenValueManualDepositPaymaster__factory(
          ethersSigner
    ...
    it('should return correct conversion', async () => {
      ...
      const requiredTokens =
        await manualDepositPaymaster.getTokenValueOfEthTest(
          token.address,
          ethBoughtAmount
        )
      // oracle returns 1:1 conversion
      expect(requiredTokens).to.be.eq(
        ethBoughtAmount.mul(10 ** priceRatioDecimals / priceRatio)
      )
    })
    it('should return correct conversion on different priceRatio values', async () => {
      ...
      let requiredTokens = await manualDepositPaymaster.getTokenValueOfEthTest(
        tokenAlt.address,
        ethBoughtAmount
      )
      ...
      requiredTokens = await manualDepositPaymaster.getTokenValueOfEthTest(
        tokenAlt.address,
        ethBoughtAmount
      )
