  ...
  describe('#validatePaymasterUserOp', () => {
    ...
    it('should reject on unsupported calldata', async () => {
      ...
      const userOp1 = await fillAndSign({
        sender: account.address,
        callData: account.interface.encodeFunctionData('execute', [token.address, 0, approveData]),
        paymasterAndData: hexConcat([paymaster.address, defaultAbiCoder.encode(['uint48', 'uint48'], [MOCK_VALID_UNTIL, MOCK_VALID_AFTER]), '0x' + '00'.repeat(65)])
      }, accountOwner, entryPoint)
    ...
    it('reject on unsupported deposit calldata', async () => {
      ...
      const userOp1 = await fillAndSign({
        sender: account.address,
        // incorrect calldata
        callData: account.interface.encodeFunctionData('execute', [depositPaymaster.address, 0, depositData]),
        paymasterAndData: hexConcat([paymaster.address, defaultAbiCoder.encode(['uint48', 'uint48'], [MOCK_VALID_UNTIL, MOCK_VALID_AFTER]), '0x' + '00'.repeat(65)])
      }, accountOwner, entryPoint)
      const hash = await paymaster.getHash(userOp1, MOCK_VALID_UNTIL, MOCK_VALID_AFTER)
      const sig = await offchainSigner.signMessage(arrayify(hash))
