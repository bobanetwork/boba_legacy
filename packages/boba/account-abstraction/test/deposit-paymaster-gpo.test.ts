...
import { MockGasPriceOracle__factory } from '../dist/types/factories/contracts/test/mocks/'
import {
  AddressZero,
  createAddress,
  createAccountOwner,
...
describe('GPODepositPaymaster', () => {
  ...
  before(async function () {
    ...
    gasPriceOracle = await new MockGasPriceOracle__factory(
      ethersSigner
    ).deploy()

    paymaster = await new GPODepositPaymaster__factory(ethersSigner).deploy(
      entryPoint.address,
      token.address,
      tokenDecimals,
  ...
  describe('deposit', () => {
    ...
    it('should deposit and read balance', async () => {
      ...
      expect(await paymaster.depositInfo(account.address)).to.eql({
        amount: 100
      })
    })
  ...
  describe('getTokenValueOfEth', () => {
    ...
    before(async () => {
      gpoDepositPaymaster =
        await new TestTokenValueGPODepositPaymaster__factory(
          ethersSigner
        ).deploy(
          entryPoint.address,
          token.address,
    ...
    it('should return correct conversion', async () => {
      ...
      const requiredTokens = await gpoDepositPaymaster.getTokenValueOfEthTest(
        ethBoughtAmount
      )
    ...
    it('should return correct conversion on different token decimals', async () => {
      ...
      let requiredTokens = await gpoDepositPaymaster.getTokenValueOfEthTest(
        ethBoughtAmount
      )
      expect(requiredTokens).to.be.eq('7812500000')

      ethBoughtAmount = ethers.utils.parseEther('0.0005')
      requiredTokens = await gpoDepositPaymaster.getTokenValueOfEthTest(
        ethBoughtAmount
      )
