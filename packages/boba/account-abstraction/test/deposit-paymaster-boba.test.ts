...
import { MockFeedRegistry__factory } from '../dist/types/factories/contracts/test/mocks/'
import { BobaDepositPaymaster__factory } from '../dist/types/factories/contracts/samples/'
...
describe('BobaDepositPaymaster', () => {
  ...
  before(async function () {
    ...
    paymaster = await new BobaDepositPaymaster__factory(ethersSigner).deploy(
      entryPoint.address,
      ethOracle.address
    )
  ...
  describe('ethOracle', () => {
    ...
    it('should not allow incorrect native token oracle', async () => {
      await expect(
        new BobaDepositPaymaster__factory(ethersSigner).deploy(
          entryPoint.address,
          ethers.constants.AddressZero
        )
      ).to.be.revertedWith('DepositPaymaster: Incorrect eth oracle')
    })
  })
  ...
  describe('addToken', () => {
    it('should not allow incorrect token oracle', async () => {
      ...
      ).to.be.revertedWith('DepositPaymaster: Incorrect token oracle')
    })
  })
  ...
  describe('deposit', () => {
    ...
    it('should fail to deposit native token', async () => {
      const nativeTokenAddress = await paymaster.L2_ETH_ADDRESS()
      await expect(
        paymaster.addDepositFor(nativeTokenAddress, account.address, 100)
      ).to.be.reverted
    })
  ...
  describe('getTokenValueOfEth', () => {
    ...
    before(async () => {
      ...
      testTokenOracle = await new MockFeedRegistry__factory(
        ethersSigner
      ).deploy()

      bobaDepositPaymaster =
        await new TestTokenValueBobaDepositPaymaster__factory(
          ethersSigner
        ).deploy(entryPoint.address, testEthOracle.address)
      // add boba token
      await bobaDepositPaymaster.addToken(
        token.address,
        testTokenOracle.address,
        token.address,
        18
      )
    })
    it('should return correct conversion', async () => {
      ...
      const requiredTokens = await bobaDepositPaymaster.getTokenValueOfEthTest(
        token.address,
        ethBoughtAmount
      )
    ...
    it('should return correct conversion on different values', async () => {
      ...
      let requiredTokens = await bobaDepositPaymaster.getTokenValueOfEthTest(
        token.address,
        ethBoughtAmount
      )
      ...
      requiredTokens = await bobaDepositPaymaster.getTokenValueOfEthTest(
        token.address,
        ethBoughtAmount
      )
