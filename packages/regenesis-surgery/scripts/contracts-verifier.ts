...
;(async () => {
  ...
  try {
    ...
    if (proxyTarget === L2LPAddress) {
      console.log(`L2 Proxy__LP check passed`)
    } else {
      console.log(`L2 Proxy__LP check failed`)
      console.log(`L2 Proxy__LP target address ${proxyTarget}`)
    }
  } catch (error) {
    console.log(`L2 Proxy__LP check failed - error ${error}`)
  }
  ...
  try {
    ...
    if (L1LPOwner.toLowerCase() === L2LPOwner.toLowerCase()) {
      console.log(`L2 LP owner check passed`)
    } else {
      console.log(`L2 LP owner check failed`)
      console.log(
        `L1 LP owner ${L1LPOwner.toLowerCase()} - L2 LP owner ${L2LPOwner.toLowerCase()}`
      )
    }
  } catch (error) {
    console.log(`L2 LP owner check failed - error ${error}`)
  }
  ...
  try {
    ...
    if (L2LPTargetL1LPAddress === L1LPContract.address) {
      console.log(`L2 LP L1LiquidityPoolAddress check passed`)
    } else {
      console.log(`L2 LP L1LiquidityPoolAddress check failed`)
      console.log(
        `L2 LP L1LiquidityPoolAddress target address ${L2LPTargetL1LPAddress}`
      )
    }
  } catch (error) {
    console.log(`L2 LP L1LiquidityPoolAddress check failed - error ${error}`)
  }

  try {
    const userRewardFeeRate = await L2LPContract.userRewardFeeRate()
    console.log(`L2 LP userRewardFeeRate ${userRewardFeeRate.toString()}`)
  } catch (error) {
    console.log(`L2 LP userRewardFeeRate check failed - error ${error}`)
  }

  try {
    const ownerRewardFeeRate = await L2LPContract.ownerRewardFeeRate()
    console.log(`L2 LP ownerRewardFeeRate ${ownerRewardFeeRate.toString()}`)
  } catch (error) {
    console.log(`L2 LP ownerRewardFeeRate check failed - error ${error}`)
  }

  try {
    const DEFAULT_FINALIZE_WITHDRAWAL_L1_GAS = await L2LPContract.DEFAULT_FINALIZE_WITHDRAWAL_L1_GAS()
    console.log(
  ...
  console.log('Verifying PoolInfo')
  for (const eachToken of Object.keys(contractList.TOKENS)) {
    const L2ERC20Address = contractList.TOKENS[eachToken].L2
    console.log(`Verifying PoolInfo of ERC20 ${eachToken}`)
    try {
      const poolData = await L2LPContract.poolInfo(L2ERC20Address)
      if (
        poolData.startTime !== 0 &&
        poolData.l1TokenAddress.toLowerCase() === contractList.TOKENS[eachToken].L1.toLowerCase() &&
        poolData.l2TokenAddress.toLowerCase() === contractList.TOKENS[eachToken].L2.toLowerCase()
      ) {
        console.log(`PoolInfo of ERC20 ${eachToken} passed`)
      } else {
        console.log(`PoolInfo of ERC20 ${eachToken} failed - data ${poolData}`)
      }
    } catch (error) {
      console.log(`PoolInfo of ERC20 ${eachToken} failed - error ${error}`)
    }
  }
})().catch((err) => {
  console.log(err)
