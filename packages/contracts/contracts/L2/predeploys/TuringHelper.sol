...
contract TuringHelper is ITuringHelper, Ownable {
    ...
    TuringHelper Self;
    BobaTuringCredit private _creditContract;
    ...
    constructor (address _bobaTuringCredit, address _owner) {
        Self = TuringHelper(address(this));
        _creditContract = BobaTuringCredit(_bobaTuringCredit);
        transferOwnership(_owner); // will be called from smart contract, transfer ownership to actual caller (TODO: maybe do abi call for context switch)
    }
    ...
    */
    ...
    function TuringTxDecentralized(string memory _command, bytes memory _payload)
    public onlyPermittedCaller returns (bytes memory) {
        (string memory url, address turingHelper) = _creditContract.getDecentralizedOracle(_command);

        // TODO: Earnings should be devoted via GETH or modifier within the turingCredit contract but assigned to the turingHelper referenced below (delegate call)
        return ITuringHelper(turingHelper).TuringTx(url, _payload); // delegate call to turingHelper responsible for apiUrl (to be eligible for earnings)
    }
