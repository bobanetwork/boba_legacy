// SPDX-License-Identifier: MIT
pragma solidity ^0.8.9;

/* Contract Imports */
import { L2StandardERC20 } from "../../standards/L2StandardERC20.sol";
import { L2StandardERC20LayerZero } from "../../standards/L2StandardERC20LayerZero.sol";
import { Lib_PredeployAddresses } from "../../libraries/constants/Lib_PredeployAddresses.sol";

/**
 * @title L2StandardTokenFactory
 * @dev Factory contract for creating standard L2 token representations of L1 ERC20s
 * compatible with and working on the standard bridge.
 */
contract L2StandardTokenFactory {
    event StandardL2TokenCreated(address indexed _l1Token, address indexed _l2Token);
    event StandardLayerZeroL2TokenCreated(address indexed _l1Token, address indexed _l2Token);

    /**
     * @dev Creates an instance of the standard ERC20 token on L2.
     * @param _l1Token Address of the corresponding L1 token.
     * @param _name ERC20 name.
     * @param _symbol ERC20 symbol.
     * @param _decimals ERC20 decimals.
     */
    function createStandardL2Token(
        address _l1Token,
        string memory _name,
        string memory _symbol,
        uint8 _decimals
    ) external {
        require(_l1Token != address(0), "Must provide L1 token address");

        L2StandardERC20 l2Token = new L2StandardERC20(
            Lib_PredeployAddresses.L2_STANDARD_BRIDGE,
            _l1Token,
            _name,
            _symbol,
            _decimals
        );

        emit StandardL2TokenCreated(_l1Token, address(l2Token));
    }

    /**
     * @dev Creates an instance of the Layer Zerostandard ERC20 token on L2.
     * @param _l1Token Address of the corresponding L1 token.
     * @param _name ERC20 name.
     * @param _symbol ERC20 symbol.
     * @param _decimals ERC20 decimals.
     */
    function createStandardLayerZeroL2Token(
        address _l1Token,
        string memory _name,
        string memory _symbol,
        uint8 _decimals
    ) external {
        require(_l1Token != address(0), "Must provide L1 token address");

        L2StandardERC20LayerZero l2Token = new L2StandardERC20LayerZero(
            Lib_PredeployAddresses.L2_STANDARD_BRIDGE,
            _l1Token,
            _name,
            _symbol,
            _decimals
        );

        // transfer ownership to sender
        l2Token.transferOwnership(msg.sender);

        emit StandardLayerZeroL2TokenCreated(_l1Token, address(l2Token));
    }
}
