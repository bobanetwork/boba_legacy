...
describe('AA Boba as Fee token Test\n', async () => {
  ...
  describe('A user without ETH pays for a tx through a paymaster that accepts $BOBA', async () => {
    ...
    it('should allow the paymaster operator to add paymaster stake (set up paymaster)', async () => {
      ...
      expect(await BobaDepositPaymaster.getDeposit()).to.be.eq(utils.parseEther('1'))
    })

    it('should allow the user to approve $BOBA token spends by the paymaster', async () => {
      ...
      expect(await L2BOBAToken.allowance(account.address, BobaDepositPaymaster.address)).to.be.eq(constants.MaxUint256)
    })
