...
describe('Boba Turing Credit Test', async () => {
  ...
  it('Should add balance to this wallet', async () => {
    const depositAmount = utils.parseEther('100')

    const preBalance = await BobaTuringCredit.prepaidBalance(env.l2Wallet.address)

    const approveTx = await L2BOBAToken.approve(BobaTuringCredit.address, depositAmount)
    await approveTx.wait()

    const depositTx = await BobaTuringCredit.addBalance(depositAmount)
    await depositTx.wait()

    const postBalance = await BobaTuringCredit.prepaidBalance(env.l2Wallet.address)

    expect(postBalance).to.be.deep.eq(preBalance.add(depositAmount))
  })

  it('Should add balance to another wallet', async () => {
    const depositAmount = utils.parseEther('100')
    ...
    const depositTx = await BobaTuringCredit.addBalanceTo(depositAmount, env.l2Wallet_2.address)
    await depositTx.wait()

    const postBalance = await BobaTuringCredit.prepaidBalance(env.l2Wallet_2.address)
